...
resource "azurerm_network_watcher_flow_log" "this" {
  enabled                   = var.enabled
  name                      = var.name_override != "" ? var.name_override : "${var.abbreviation}-${var.application_name}-${var.subscription_type}-${var.environment}-${var.location}-${random_id.id.hex}"#-${var.instance_number}-${random_id.id.hex}
  ...
  lifecycle {
    ignore_changes = [location, tags, name]
  }
}